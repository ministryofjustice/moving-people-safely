class ConsiderationsFormBuilder < GovukElementsFormBuilder::FormBuilder
  def radio_toggle(attribute, attribute_with_error = nil, choices = nil, &_blk)
    style = 'optional-section-wrapper'
    style << ' mps-hide' unless object.public_send("#{attribute}").on?
    ...
    choices ||= object.public_send(attribute).toggle_choices
  ...
  def field_without_label(field_type, attribute)
    ...
    content_tag :div,
      ...
      field_tag =
        field_type.new(
          object.name, attribute_details, self,
