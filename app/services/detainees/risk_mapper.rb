module Detainees
  class RiskMapper
    ...
    def call
      {
        ...
        intimidation_to_public: alert_value(ALERT_CODES[:intimidation_to_public], :boolean),
        intimidation_to_public_details: alert_comments(ALERT_CODES[:intimidation_to_public]),
        intimidation_to_other_detainees: alert_value(ALERT_CODES[:intimidation_to_other_detainees], :boolean),
        ...
        other_violence_due_to_discrimination: alert_value(ALERT_CODES[:other_violence_due_to_discrimination]),
    ...
    def alert_value(codes, return_type = :string)
      if return_type == :string
        alerts_for_codes(codes).any? ? 'yes' : 'no'
      elsif return_type == :boolean
        alerts_for_codes(codes).any? ? true : false
      end
    end

    def alert_comments(codes)
      alerts_for_codes(codes).map { |alert| alert['comment'] }.join(' | ')
    end
