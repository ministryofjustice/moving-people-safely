...
RSpec.describe Forms::Risks::Communication, type: :form do
  let(:model) { Risks.new }
  ...
  let(:params) {
    {
      ...
      interpreter_required_details: 'German',
  ...
  describe '#validate' do
    describe 'nilifies empty strings' do
      %w[ interpreter_required_details hearing_speach_sight_details can_read_and_write_details ].each do |attribute|
    ...
    context 'when interpreter_required is set to yes' do
      ...
      it { is_expected.to validate_presence_of(:interpreter_required_details) }
    end
