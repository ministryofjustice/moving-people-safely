...
RSpec.feature 'searching for a prisoner', type: :feature do
  ...
  context 'from police' do
    ...
    context 'with a valid pnc number' do
      ...
      scenario 'prisoner present with an active escort' do
        escort = create(:escort, pnc_number: pnc_number, from_establishment: PoliceCustody.first)
      ...
      scenario 'prisoner present with a previously issued escort' do
        escort = create(:escort, :issued, pnc_number: pnc_number, from_establishment: PoliceCustody.first)
  ...
  def expect_result_with_move(escort)
    expect(page).to have_content(escort.number).
      and have_content(escort.surname).
      and have_content(escort.date_of_birth.strftime('%d %b %Y')).
      and have_content(escort.to).
      and have_content(escort.date.strftime('%d %b %Y'))
  end
